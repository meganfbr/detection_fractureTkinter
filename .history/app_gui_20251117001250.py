...
if os.path.exists(MODEL1_PATH):
    print(f"File model ditemukan: {MODEL1_PATH}")
else:
    print(f"File model tidak ditemukan di: {MODEL1_PATH}")
...
def safe_load_model_with_compat(path):
    ...
    try:
        # Primary attempt to load the model
        return load_model(path, compile=False)
    except Exception as e:
        # Second attempt with custom objects for InputLayer
        try:
            from tensorflow.keras import layers
            return load_model(path, compile=False, custom_objects={
                "InputLayer": layers.InputLayer,
                "BatchNormalization": layers.BatchNormalization  # Add other layers if needed
            })
