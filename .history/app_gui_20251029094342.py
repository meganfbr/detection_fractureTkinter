...
# ======================================================
# ⚙️ DETEKSI GAMBAR X-RAY (VERSI FINAL - ANTI FOTO & TOLERAN X-RAY WEB)
# ======================================================
# ======================================================
# ⚙️ DETEKSI GAMBAR X-RAY (FINAL FIXED - ANTI FOTO BIRU & FOTO MANUSIA)
# ======================================================
def is_xray_image(img_path):
    ...
    s_thr = adaptive_stats["sat_mean"] + 3.0 * adaptive_stats["sat_std"]
    ...
    # 🔹 1. Tolak gambar terlalu jenuh atau terlalu berwarna
    if sat > s_thr or color_diff > 40:
        print("❌ Ditolak: warna terlalu jenuh (kemungkinan bukan X-ray).")
        return False

    # 🔹 2. Tolak gambar terlalu terang, gelap, atau kontras sangat rendah
    if bright < b_thr_low or bright > b_thr_high:
        print("❌ Ditolak: kecerahan tidak sesuai citra medis.")
        return False
    if contrast < c_thr_low:
        print("❌ Ditolak: kontras terlalu rendah (kemungkinan foto biasa).")
        return False

    # 🔹 3. Tolak pola warna kulit (R > G > B)
    if mean_bgr[2] > mean_bgr[1] > mean_bgr[0] and mean_bgr[2] - mean_bgr[0] > 15:
        print("❌ Ditolak: pola warna kulit terdeteksi (foto manusia).")
        return False

    # 🔹 4. Izinkan X-ray dengan dominan biru TAPI hanya jika saturasi rendah dan kontras tinggi
    if mean_bgr[0] > mean_bgr[1] and mean_bgr[0] > mean_bgr[2]:
        if sat < 40 and contrast > c_thr_low:
            print("✅ X-ray berwarna kebiruan (masih valid).")
            return True
        else:
            print("❌ Biru terlalu kuat — bukan X-ray medis.")
            return False

    # 🔹 5. Range abu-abu khas X-ray
    gray_mean = np.mean(gray)
    if 35 < gray_mean < 220:
        print("✅ Valid X-ray terdeteksi.")
        return True

    print("❌ Tidak memenuhi karakteristik citra X-ray.")
    return False
    ...
    sat = np.mean(hsv[:, :, 1])            # saturasi rata-rata (seberapa berwarna)
    bright = np.mean(hsv[:, :, 2])         # kecerahan rata-rata
    contrast = np.std(gray)                # kontras
    mean_bgr = np.mean(img, axis=(0, 1))   # rata-rata nilai warna RGB
    color_diff = np.max(mean_bgr) - np.min(mean_bgr)  # beda antar channel warna

    # Threshold adaptif dari sample dataset kamu
    s_thr = adaptive_stats["sat_mean"] + 3.0 * adaptive_stats["sat_std"]
    ...
    # 🔹 1. Deteksi X-ray berwarna biru → masih dianggap valid
    if mean_bgr[0] > mean_bgr[1] and mean_bgr[0] > mean_bgr[2] and mean_bgr[0] - mean_bgr[2] > 10:
        print("✅ Warna kebiruan khas X-ray terdeteksi (masih valid).")
        return True

    # 🔹 2. Tolak gambar terlalu berwarna atau saturasi tinggi
    if sat > s_thr and color_diff > 45:
        print("❌ Ditolak: warna terlalu jenuh atau dominan (kemungkinan bukan X-ray).")
        return False

    # 🔹 3. Tolak gambar terlalu terang/gelap atau kontras sangat rendah
    if bright < b_thr_low or bright > b_thr_high:
        print("❌ Ditolak: tingkat kecerahan tidak sesuai citra medis.")
        return False
    if contrast < c_thr_low:
        print("❌ Ditolak: kontras terlalu rendah (kemungkinan foto biasa).")
        return False

    # 🔹 4. Tolak gambar dengan pola warna kulit (R > G > B)
    ...
    # 🔹 5. Cek grayscale range (abu-abu khas X-ray)
    gray_mean = np.mean(gray)
    if gray_mean < 25 or gray_mean > 235:
        print("❌ Ditolak: gambar terlalu gelap/terlalu terang untuk X-ray.")
        return False

    # 🔹 6. Jika warnanya masih agak tinggi tapi dominan biru → auto-convert ke grayscale
    if sat > s_thr:
        print("🔧 Otomatis ubah X-ray berwarna menjadi grayscale untuk diproses.")
        gray_fixed = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        cv2.imwrite("temp_fixed_gray.jpg", gray_fixed)
        return True
