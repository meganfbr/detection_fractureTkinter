...
class XrayClassifierApp(QtWidgets.QWidget):
    ...
    def run_prediction(self, img_path):
        img_arr = preprocess_image(img_path, "model1")  # For model 1
        ...
        # Model 1 prediction: Check if it's non-X-ray and stop processing
        ...
        # If it's an X-ray, use Model 2
        img_arr = preprocess_image(img_path, "model2")  # For model 2
        ...
        result_text = (
            ...
            f"Precision : {conf2:.4f}\n"
            f"Recall    : {conf2:.4f}\n"
            f"F1-score  : {(2 * conf2 * conf2) / (conf2 + conf2 + 1e-12):.4f}\n"
            f"Avg Recall: {np.mean(pred2):.4f}\n"
        )
