...
def is_xray_image(img_path):
    ...
    # --- Statistik dasar
    sat_mean = np.mean(hsv[:, :, 1])        # seberapa berwarna
    bright_mean = np.mean(hsv[:, :, 2])     # kecerahan
    contrast = np.std(gray)                 # kontras abu-abu

    # --- Dominasi warna (hindari biru, merah, kuning mencolok)
    mean_bgr = np.mean(img, axis=(0, 1))
    blue_ratio = mean_bgr[0] / (np.sum(mean_bgr) + 1e-5)
    red_ratio = mean_bgr[2] / (np.sum(mean_bgr) + 1e-5)

    # --- Logika deteksi X-ray
    is_gray_like = sat_mean < 40 and contrast > 15
    not_colorful = blue_ratio < 0.4 and red_ratio < 0.4
    bright_ok = 40 < bright_mean < 230

    if is_gray_like and not_colorful and bright_ok:
        print(f"✅ X-ray valid (sat={sat_mean:.1f}, bright={bright_mean:.1f}, contrast={contrast:.1f})")
        return True
    else:
        print(f"❌ Bukan X-ray (sat={sat_mean:.1f}, bright={bright_mean:.1f}, contrast={contrast:.1f}, blue={blue_ratio:.2f}, red={red_ratio:.2f})")
        return False
