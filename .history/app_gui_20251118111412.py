...
# ---------------- CONFIG ----------------
MODELS_DIR = "."  # folder tempat file ada; sesuaikan kalau perlu
MODEL1_PATH = os.path.join(MODELS_DIR, "model1_xray_filter.h5")
MODEL2_PATH = os.path.join(MODELS_DIR, "fractureCNNRev_model.h5")
CLASS1_JSON = os.path.join(MODELS_DIR, "class_names_model1.json")
CLASS2_JSON = os.path.join(MODELS_DIR, "class_names_model2.json")  # <-- perintahmu
...
def preprocess_image(img_path):
    # Use PIL to be robust to cv2 color issues
...
class XrayClassifierApp(QtWidgets.QWidget):
    def __init__(self):
        ...
        try:
            ...
            print("Loaded Model1:", MODEL1_PATH)
        except Exception as e:
        ...
        try:
            ...
            print("Loaded Model2:", MODEL2_PATH)
        except Exception as e:
        ...
        # top-3 probabilities display (table-like)
        ...
        # Global evaluation (static) display
        ...
        # Apply global metrics if present
        ...
        # keep references to popup windows to avoid GC
        self._popup_windows = []
    ...
    def show_global_metrics(self):
        ...
        if self.cm_exists:
            try:
                pix = QtGui.QPixmap(CM_PNG_PATH).scaled(420, 320, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
    ...
    def load_image(self):
        ...
        try:
            # display image
            qpix = QtGui.QPixmap(file_path).scaled(*IMG_DISPLAY_SIZE, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
    ...
    def run_prediction(self, img_path):
        ...
        # Model2: fracture classification
        ...
        # Dynamic metrics per image (probability-based)
        probs = np.array(pred2, dtype=float)
        p = probs[idx2]  # probability for predicted class (0..1)
        dynamic_precision = float(p)
        dynamic_recall = float(p)
        dynamic_f1 = float((2 * p * p) / (p + p + 1e-12))  # simplifies to p but keep formula
        ...
        # Fill top-3 list with class names and probs
        self.top3_list.clear()
        ...
        # Show single-sample confusion matrix popup if no global CM
    ...
    def show_single_cm(self, pred_idx, num_classes):
        try:
            ...
            # create popup window and keep reference
            cm_window = QtWidgets.QWidget()
            cm_window.setWindowTitle("Confusion Matrix (single-sample view)")
            v = QtWidgets.QVBoxLayout()
            lbl = QtWidgets.QLabel()
            pix = QtGui.QPixmap(tmpfile).scaled(720, 520, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
            lbl.setPixmap(pix)
            v.addWidget(lbl)
