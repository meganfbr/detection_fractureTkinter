...
# ======================================================
# ⚙️ DETEKSI GAMBAR X-RAY (VERSI FINAL — LEBIH AKURAT)
# ======================================================
def is_xray_image(img_path):
    ...
    sat = np.mean(hsv[:, :, 1])           # rata-rata kejenuhan warna
    bright = np.mean(hsv[:, :, 2])        # rata-rata kecerahan
    contrast = np.std(gray)               # deviasi standar (kontras)

    # threshold adaptif (sedikit lebih ketat untuk non-Xray)
    s_thr = adaptive_stats["sat_mean"] + 2.5 * adaptive_stats["sat_std"]
    ...
    c_thr_low = adaptive_stats["contrast_mean"] - 1.5 * adaptive_stats["contrast_std"]

    # Jika warnanya dominan & kontras terlalu kecil → bukan X-ray
    if sat > s_thr or contrast < c_thr_low:
        return False

    # Kalau terlalu berwarna (R,G,B berbeda jauh), berarti bukan X-ray
    mean_bgr = np.mean(img, axis=(0, 1))
    color_diff = np.max(mean_bgr) - np.min(mean_bgr)
    if color_diff > 40:  # makin tinggi makin ketat
        return False

    # Terakhir, cek apakah grayscale-nya dalam kisaran normal X-ray
    gray_ratio = np.mean(gray)
    if gray_ratio < 25 or gray_ratio > 235:
        return False

    # Kalau semua lolos, berarti ini valid X-ray
    return True
