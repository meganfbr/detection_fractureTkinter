...
class XrayClassifierApp(QtWidgets.QWidget):
    ...
    def run_prediction(self, img_path):
        # preprocess untuk model 1
        img1 = preprocess_image_model1(img_path)

        pred1 = self.model1.predict(img1, verbose=0)[0]
        idx1 = int(np.argmax(pred1))
        class1_name = safe_get_classname(self.class1, idx1)
        conf1 = float(pred1[idx1]) * 100

        # jika X-ray → lanjut ke model 2
        img2 = preprocess_image_model2(img_path)

        pred2 = self.model2.predict(img2, verbose=0)[0]
